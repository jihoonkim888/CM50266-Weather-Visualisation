...
def read_csv(path, filename, codec, sep=','):
    ...
    # unexpected character in a numeric field...
...
# calculating and printing features of each component...
f = open("task1-features.txt", 'w')
path = './lab1-data/'
...
# writing minimum, maximum, mean and standard deviation for each component to task1-features.txt
...
# Collect all the data columns in several files in one pandas dataframe...
df = pd.DataFrame()
for filename in filenames:
    header, data = read_csv(path, filename, 'utf-8-sig')
...
# separate dataframe for corr matrix...
df_corr = df.loc[:, ['DateTime', 'Baro', 'Humidity indoor', 'Temperature indoor', 'Temperature outside', 'mm']]
# print('Features:\n', df_corr.describe())
corr = df_corr.corr()
...
plt.tight_layout()
plt.savefig('corr_scatter_5x5.svg')
plt.show()
...
plt.tight_layout()
plt.savefig('corr_heatmap_5x5.svg')
plt.show()

# ''' another correlation matrix with min and max values '''
